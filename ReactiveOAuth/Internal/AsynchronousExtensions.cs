...
namespace Codeplex.OAuth
{
	...
	internal static class WebRequestExtensions
	{
		...
		{
			return Observable.Defer(() => request.GetResponseAsync().ToObservable()).SelectMany(r => r.DownloadDataAsync());
		}

		public static IObservable<Progress<byte[]>> DownloadDataAsyncWithProgress(this WebRequest request, int chunkSize = 65536)
		{
			return Observable.Defer(() => request.GetResponseAsync().ToObservable()).SelectMany(r => r.DownloadDataAsyncWithProgress(chunkSize));
		}
		...
		{
			return Observable.Defer(() => request.GetResponseAsync().ToObservable()).SelectMany(r => r.DownloadStringAsync(encoding));
		}
		...
		{
			return Observable.Defer(() => request.GetResponseAsync().ToObservable()).SelectMany(r => r.DownloadStringLineAsync(encoding));
		}
		...
		{
			return Observable.Defer(() => request.GetRequestStreamAsync().ToObservable())
				.SelectMany(stream => stream.WriteAsync(data, 0, data.Length).ToObservable().Finally(() => stream.Close()))
				.TakeLast(1)
				.SelectMany(_ => request.GetResponseAsync().ToObservable());
		}
	...
	internal static class StreamExtensions
	{
		...
		public static IObservable<Unit> WriteAsync(this Stream stream, IObservable<byte> data, int chunkSize = 65536)
		{
			return Observable.Defer(() => data)
				.Buffer(chunkSize)
				.SelectMany(l => stream.WriteAsync(l.ToArray(), 0, l.Count).ToObservable())
		...
		public static IObservable<byte[]> ReadAsync(this Stream stream, int chunkSize = 65536)
		{
			return Observable.Defer(() => Observable.Return(new byte[chunkSize], Scheduler.CurrentThread))
				.SelectMany(
					buffer => stream.ReadAsync(buffer, 0, chunkSize).ToObservable(),
